...
>>>>>>> 50b7df0a5f086cbcf64800427a62fc46763c4204
      ...
      public bool MsaglGoodResolution(Vertex w, int[,] listNeighbors, int numNeighbors, int offset)
      {
         for (int i = 1; i < numNeighbors; i++)
         {
            ...
            for (int index = 0; index < candidateVertex.Length; index++)
            {
               ...
               if (z.Id == w.Id || z.Id == VList[listNeighbors[i, 1]].Id)
                  continue;
      ...
      public bool GoodResolution(Vertex w, int[,] listNeighbors, int numNeighbors, WeightedPoint[] pt, int numPoints)
      {
         for (int i = 1; i <= numNeighbors; i++)
         {
            ...
            {
               if (z.Id == w.Id || z.Id == VList[listNeighbors[i, 1]].Id || z.Invalid)
                  continue;
      ...
      public bool IsWellSeperated(Vertex w, int w1, int w2, WeightedPoint[] pt, int numPoints)
      {
         ...
         {
            if (VList[pt[index].GridPoint].Invalid)
               continue;
            if ((pt[index].X == VList[w1].XLoc && pt[index].Y == VList[w1].YLoc)
                || (pt[index].X == VList[w2].XLoc && pt[index].Y == VList[w2].YLoc))
               continue;
            ...
            */
            ...
            for (int neighb = 1; neighb <= DegList[w.Id]; neighb++)
               if (EList[w.Id, neighb].NodeId == w1)
               { k = neighb; break; }
         ...
         {
            if (EList[w1, neighb].Used == 0)
               continue;
            if (EList[w1, neighb].NodeId == w.Id || EList[w1, neighb].NodeId == w2)
               continue;
         ...
         {
            if (EList[w2, neighb].Used == 0)
               continue;
            if (EList[w2, neighb].NodeId == w.Id || EList[w2, neighb].NodeId == w1)
               continue;
      ...
      public bool MsaglIsWellSeperated(Vertex w, int w1, int w2)
      {
         ...
         for (int q = 0; q < candidateList.Length; q++)
         {
            ...
            if (VList[index].Invalid || DegList[index] == 0 || VList[index].Id == w1 || VList[index].Id == w2 || VList[index].Id == w.Id)
               continue;
      ...
      */
      ...
      public void MsaglDetour(Dictionary<int, Node> idToNode, bool omit)
      {
         ...
         for (int index = 0; index < N; index++)
         {
            ...
            for (neighbor = 0; neighbor < DegList[index]; neighbor++)
            {
               ...
               if (w.YLoc == b.YLoc && w.XLoc > b.XLoc)
               {
                  ...
                  if (exists == -1)
                  {
                     ...
                     removelist[remove, 0] = w.Id;
                     removelist[remove, 1] = b.Id;
                     remove++;
                     addlist[add, 0] = w.Id;
                     addlist[add, 1] = a;
                     addlist[add, 2] = b.Id;
            ...
            for (int i = 0; i < remove; i++)
               RemoveEdge(removelist[i, 0], removelist[i, 1]);
            for (int i = 0; i < add; i++)
            {
      ...
      public void ComputeShortcutMesh(WeightedPoint[] pt, int numPoints)
      {
         ...
         for (int i = numPoints; i >= 1; i--)
         {
            ...
            while (x + 1 < N && y + 1 < N && NodeMap[x + 1, y + 1] > 0)
            {
               ...
               {
                  if (EList[NodeMap[x, y], neighb].NodeId == VList[NodeMap[x + 1, y + 1]].Id)
                     break;
               }
               if (EList[NodeMap[x, y], neighb].Selected == 0)
                  break;
               x = x + 1;
               y = y + 1;
            }
      ...
      {
         for (int index = 0; index < NumOfnodes; index++)
            if (a == VList[index].XLoc && b == VList[index].YLoc && VList[index].Invalid == false)
               return index;
         return -1;
      }
      ...
      {
         for (int index = 0; index < NumOfnodes; index++)
            if (a == VList[index].XLoc && b == VList[index].YLoc && VList[index].Invalid == false && index != givenNodeId)
               return index;
         return -1;
      }
      ...
      public bool RemoveEdge(int a, int b)
      {
         ...
         for (int index = 0; index < DegList[b]; index++)
         {
            if (EList[b, index].NodeId == a)
            {
               DegList[b]--;
               i++;
               for (; index < DegList[b]; index++)
                  EList[b, index] = EList[b, index + 1];
            }
         }
