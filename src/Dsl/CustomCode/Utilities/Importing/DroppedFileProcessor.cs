...
namespace Sawczyn.EFDesigner.EFModel {
   public class DroppedFileProcessor : DropProcessor
   {
      private readonly string filename;

      public DroppedFileProcessor(Store store, string filename) : base(store)
      {
      ...
      private void ProcessIdentities(List<ModelClass> processedClasses)
      {
            ...
            foreach (PropertyDeclarationSyntax propertyDecl in classDecl.DescendantNodes().OfType<PropertyDeclarationSyntax>())
            {
               ...
               try
               {
                  ...
                  ModelAttribute modelAttribute =
                     new ModelAttribute(store, new PropertyAssignment(ModelAttribute.NameDomainPropertyId, propertyName))
                     {
      ...
      private void ProcessAssociation([NotNull] ModelClass source, [NotNull] ModelClass target,
      ...
      private ModelEnum ProcessEnum([NotNull] EnumDeclarationSyntax enumDecl, NamespaceDeclarationSyntax namespaceDecl = null)
      {
         ...
         try
         {
            result = new ModelEnum(store, new PropertyAssignment(ModelEnum.NameDomainPropertyId, enumName))
                     {
