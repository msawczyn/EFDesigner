...
namespace Sawczyn.EFDesigner.EFModel.DslPackage.TextTemplates.EditingOnly
{
   ...
   partial class EditOnly
   {
      ...
      public abstract partial class EFModelGenerator
      {
         protected ModelRoot modelRoot { get; }
         protected Manager manager { get; protected private set; }

         protected abstract List<string> GetAdditionalUsingStatements();
         protected abstract string[] SpatialTypes { get; }

         public abstract void Generate();

         protected EFModelGenerator(Manager manager, ModelRoot modelRoot)
         {
            this.manager = manager;
            this.modelRoot = modelRoot;
         }

         protected string[] NonNullableTypes
         {
            get
            {
               return new[]
         ...
         protected void WriteDefaultConstructorBody(ModelClass modelClass)
         {
            ...
            foreach (NavigationProperty navigationProperty in modelClass.LocalNavigationProperties()
                                                                        .Where(x => x.AssociationObject.Persistent
                                                                                 ...
                                                                                 && (modelClass.ModelRoot.IsEFCore5Plus || x.Required)
                                                                                 && !x.IsCollection
